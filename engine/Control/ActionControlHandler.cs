...
/// </param>
public sealed class ActionControlHandler<TAction>(Func<TAction, byte> getPriority)
    where TAction : struct, Enum
{
    ...
    {
        if (remainingTime is null) return;
        ...
        out bool success)
    {
        ...
        var currentPriority = this.getPriority(currentAction.Value);
        var newPriority = this.getPriority(desiredAction);
    ...
    public TAction TryUpdateAction(
        ...
        TAction desiredAction,
        out bool success)
        => TryUpdateAction(ref currentAction, ref remainingTime, desiredAction, 0f, out success);
}